...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      let Num = 0
      console.log(Num, cart.skuNum, disNum)
      if (type === "minus") {
        if (cart.skuNum < 1) {
          Num = 0
        }
        ...
        if (isNaN(disNum) || disNum < 1) {
          ...
          Num = parseInt(disNum) - cart.skuNum
        }
      }
    ...
    async deleteChoose() {
      await Promise.all(
        this.cartInfoList.map(async (cart) => {
          if (cart.isChecked) {
            await this.$store.dispatch("deleteCart", cart.skuId)
          }
        }),
